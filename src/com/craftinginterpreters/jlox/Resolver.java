...
public class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
	...
	public Void visitBreakStmt(Stmt.Break stmt) {
		if (currentBreak == BreakType.NONE) { // If we aren't inside anything, then using 'return' is an error
			Lox.error(stmt.token, "Cannot break from outside a loop or do statement.");
		}
		return null;
	}
	...
	public Void visitWhileStmt(Stmt.While stmt) {
		BreakType enclosingBreak = currentBreak; // save current enclosing break type
		currentBreak = BreakType.LOOP; // Update current 'within-a-breakable' state

		resolve(stmt.condition);
		resolve(stmt.body);

		currentBreak = enclosingBreak; // Restore 'within-a-breakable' state
		return null;
	}
